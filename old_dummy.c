...
int asgn2_open(struct inode *inode, struct file *filp) {
  ...
  //if device already opened by different process, this process is put in to 
  //a wait queue.
  ...
  //Only allows for read-only access
...
int asgn2_release (struct inode *inode, struct file *filp) {
  ...
  //Wake up next process in the wait queue
...
void readbuf_fun (unsigned long t_arg) {
	size_t size_writtern = 0;    /*size to be written to virtual disk inthis function*/
	...
	size_t curr_page_offset = page_queue.tail_offset; /*offset in current page*/
